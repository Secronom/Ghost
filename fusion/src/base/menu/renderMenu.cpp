...
void Menu::RenderMenu()
{
	...
	ImVec2 textSize = Menu::FontBold->CalcTextSizeA(30, FLT_MAX, 0.0f, "333");
	float posX = windowPos.x + (columnWidth / 2) - (textSize.x / 2);
	float posY = windowPos.y + 12;

	Menu::GlitchText("333", ImVec2(posX, posY));
	ImGui::SetCursorPosY(textSize.y + 30);

	if (Menu::TabButton("CLICKER", (currentTab == 2 ? ImVec4(0.9f, 0.3f, 0.3f, 0.2f) : ImVec4(0.1f, 0.1f, 0.1f, 0.f)))) currentTab = 2;
	if (Menu::TabButton("COMBAT", (currentTab == 1 ? ImVec4(0.9f, 0.3f, 0.3f, 0.2f) : ImVec4(0.1f, 0.1f, 0.1f, 0.f)))) currentTab = 1;
	if (Menu::TabButton("VISUAL", (currentTab == 0 ? ImVec4(0.9f, 0.3f, 0.3f,0.2f) : ImVec4(0.1f, 0.1f, 0.1f, 0.f)))) currentTab = 0;
