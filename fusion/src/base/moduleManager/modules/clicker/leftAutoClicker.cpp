...
void LeftAutoClicker::Update()
{
	...
	// Check if ANY GUI >:( is open and allow clicking if `enableInvClicker` is true
	if (SDK::Minecraft->IsInGuiState() && !enableInAllGUIs) return;
...
void LeftAutoClicker::RenderMenu()
{
	...
	if (ImGui::BeginChild("autoclicker", ImVec2(425, 145))) {
		ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 3);
		Menu::DoToggleButtonStuff(857834, "LMB Auto Clicker", &LeftAutoClicker::Enabled);
		...
		Menu::DoToggleButtonStuff(333666, "Enable in All GUI's", &LeftAutoClicker::enableInAllGUIs);
